...
export function ProductOptionGroupsEditor({ productId }: Props) {
  ...
  const addOption = (groupIndex: number) => {
    setLocalGroups(prev => prev.map((g, i) =>
      i === groupIndex ? { ...g, options: [...g.options, { name: '', price: 0 }] } : g
    ));
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Soft-delete all existing groups and options for this product
  ...
  return (
    <div className="space-y-4 border border-border/50 rounded-xl p-4 bg-muted/30">
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-sm">Blocos de Escolhas</h3>
      ...
      {localGroups.length === 0 && (
        <p className="text-xs text-muted-foreground text-center py-4">Nenhum bloco de escolhas. Clique em "Novo Bloco" para começar.</p>
      )}
      ...
      {localGroups.length > 0 && (
        <Button type="button" className="w-full rounded-xl" onClick={handleSave} disabled={saving}>
          {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Salvar Opções'}
        </Button>
      )}
    </div>
  );
